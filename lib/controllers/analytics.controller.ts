...
export const analyticsController = {
    getOverview: async (req: Request, res: Response) => {
        try {
            ...
            const result = {
                total: total[0].count,
                open: stats.find((s: StatusCount) => s.status === "OPEN")?.count || 0,
                in_progress: stats.find((s: StatusCount) => s.status === "IN_PROGRESS")?.count || 0,
                resolved: stats.find((s: StatusCount) => s.status === "RESOLVED")?.count || 0,
                closed: stats.find((s: StatusCount) => s.status === "CLOSED")?.count || 0,
            };
    ...
    getStatsOverview: async (req: Request, res: Response) => {
        try {
            const stats = await db
                ...
                .from(ticket)
                .groupBy(ticket.status, ticket.priority);
